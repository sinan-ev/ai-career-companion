...
def build_datasets(
    ...
    """
    Generates tailored analytics and machine-learning-ready datasets from processed DataFrames.

    Args:
        df_analytics (pd.DataFrame): A snapshot of the DataFrame prior to categorical encoding.
        df_ml (pd.DataFrame): The fully processed DataFrame, including encoding and scaling.
        module1_output (dict): The complete context dictionary generated by Module 1.
        artifacts (dict): The dictionary of accumulated preprocessing artifacts, like scalers and dropped columns.

    Returns:
        Dict[str, Any]: A dictionary containing the two final datasets, target details, and dataset shapes.
    """
...
def _compute_target_stats(df: pd.DataFrame, target_col: str) -> dict:
    """
    Computes summary statistics regarding the identified target column to provide analytical context.

    Args:
        df (pd.DataFrame): The machine-learning-ready DataFrame.
        target_col (str): The name of the target column.

    Returns:
        dict: A dictionary of basic statistics such as class balance (for classification) or descriptive stats (for regression).
    """
